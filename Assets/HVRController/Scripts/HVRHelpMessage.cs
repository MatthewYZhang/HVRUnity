...
public class HVRHelpMessage : MonoBehaviour
{
    ...
    private Text m_trigger_text, m_volume_text, m_back_short_text, m_back_long_text, m_back_double_text, m_home_short_text, m_home_long_text, m_home_double_text, m_confirm_text;
    ...
    public static string m_back_double_msg = null;
    public static string m_home_short_msg = null;
    public static string m_home_long_msg = null;
    public static string m_home_double_msg = null;
    ...
    private string m_BackDoubleMsg = "切换背后视角（双击）";
    private string m_HomeShortMsg = "返回主页（短按）";
    private string m_HomeLongMsg = "视角及射线置中（长按）";
    private string m_HomeDoubleMsg = "视角及射线置中（双击）";
    ...
    private Transform m_noloHelpMessage;
    ...
    private void Awake()
    {
        ...
        m_noloHelpMessage = transform.Find("NoloHelpMessage");
    ...
    void Start()
    {
        if (Application.platform == RuntimePlatform.Android)
        {
            if (HVRPluginCore.IsPhoneController())
            {
                ...
                m_noloHelpMessage.gameObject.SetActive(false);
                m_helpMessage = m_phoneHelpMessage;
                InitPhoneControllerMsg();
            } else if (HVRController.m_UseNoloController) {
                HVRLogCore.LOGI(TAG, "show nolo controller help msg");
                m_IsPhoneControllerMode = false;
                m_controllerHelpMessage.gameObject.SetActive(false);
                m_phoneHelpMessage.gameObject.SetActive(false);
                m_noloHelpMessage.gameObject.SetActive(true);
                m_helpMessage = m_noloHelpMessage;
                InitNoloControllerMsg();
            }
            else
            {
    ...
    private void FindObj()
    {
        ...
        m_back_double_text = m_back.Find("double/Text").GetComponent<Text>();
        m_home_short_text = m_home.Find("short/Text").GetComponent<Text>();
        m_home_long_text = m_home.Find("long/Text").GetComponent<Text>();
        m_home_double_text = m_home.Find("double/Text").GetComponent<Text>();
    ...
    {
        FindObj();
        GetPhoneControllerMsg();
    }

    private void GetPhoneControllerMsg()
    {
        if (Application.platform == RuntimePlatform.Android)
        {
    ...
    private void InitDefaultControllerMsg()
    {
        ...
        m_trigger = m_helpMessage.transform.Find("Trigger");
        m_trigger_text = m_trigger.Find("Text").GetComponent<Text>();
        GetDefaultControllerMsg();
    }

    private void GetDefaultControllerMsg() {
        if (Application.platform == RuntimePlatform.Android) {
    ...
    private void GetNoloControllerMsg()
    {
        if (Application.platform == RuntimePlatform.Android)
        {
            m_TriggerMsg = HVRPluginCore.GetDialogueContent("Hvr_trigger");
            ...
            m_BasckShortMsg = HVRPluginCore.GetDialogueContent("Hvr_back_short");
            m_BackLongMsg = HVRPluginCore.GetDialogueContent("Hvr_back_long");
            m_BackDoubleMsg = HVRPluginCore.GetDialogueContent("Hvr_back_double");
            m_HomeShortMsg = HVRPluginCore.GetDialogueContent("Hvr_home_short");
            m_HomeLongMsg = HVRPluginCore.GetDialogueContent("Hvr_home_long");
            m_HomeDoubleMsg = HVRPluginCore.GetDialogueContent("Hvr_home_double");
            m_ConfirmMsg = HVRPluginCore.GetDialogueContent("Hvr_trigger");
        }
    }
    ...
    void Update()
    {
        ...
        if (HVRController.m_UseNoloController && m_helpMessage != m_noloHelpMessage)
        {
            HVRLogCore.LOGI(TAG, "switch to show nolo controller help msg");
            m_IsPhoneControllerMode = false;
            m_controllerHelpMessage.gameObject.SetActive(false);
            m_phoneHelpMessage.gameObject.SetActive(false);
            m_noloHelpMessage.gameObject.SetActive(true);
            m_helpMessage = m_noloHelpMessage;
            InitNoloControllerMsg();
        } else if (m_IsPhoneControllerMode) {
            if (Application.platform == RuntimePlatform.Android)
            {
                if (HVRNoticeCore.m_IsLanguageChanged)
                {
                    HVRNoticeCore.m_IsLanguageChanged = false;
                    GetPhoneControllerMsg();
                }
    ...
    private void DefaultOrCustomize()
    {
        ...
        if (!string.IsNullOrEmpty(m_back_double_msg))
        {
            m_back_double_text.text = m_back_double_msg;
        }
        else
        {
            m_back_double_text.text = m_BackDoubleMsg;
        }
        ...
        if (!string.IsNullOrEmpty(m_home_double_msg))
        {
            m_home_double_text.text = m_home_double_msg;
        }
        else
        {
            m_home_double_text.text = m_HomeDoubleMsg;
        }
