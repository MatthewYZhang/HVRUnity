...
public class HVRController : MonoBehaviour
{
    ...
    private GameObject m_NoloControllerObj;
    private GameObject m_ControllerObj;

    public static GameObject m_RightControllerObj;
    public static GameObject m_LeftControllerObj;
    ...
    public static bool m_UseNoloController = false;
    ...
    private void Awake()
    {
        ...
        m_NoloControllerObj = transform.Find("NOLOController").gameObject;
    ...
    private void ActivateAvailableController() {
        ...
        {
            string prod_model = "";
            HvrApi.GetControllerHandle().GetValue("prod_model", ref prod_model, 20);
            m_UseNoloController = prod_model.Equals("nolo cv1 air");
            if (HVRPluginCore.IsPhoneController())
            {
                ...
                m_NoloControllerObj.SetActive(false);
                m_ControllerObj = m_PhoneControllerObj;
            } else if (m_UseNoloController) {
                HVRLogCore.LOGI(TAG, "use NoloController mode");
                m_IsPhoneControllerMode = false;
                m_DefaultControllerObj.SetActive(false);
                m_PhoneControllerObj.SetActive(false);
                m_NoloControllerObj.SetActive(true);
                m_ControllerObj = m_NoloControllerObj;
            }
            else
            {
                m_IsPhoneControllerMode = false;
    ...
    private void GetControllerHandle() {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            if (gameObject.name.Equals("HVRLeftController"))
            {
                ...
                m_LeftControllerObj = gameObject;
    ...
    {
        if (m_Controller == null && gameObject.name.Equals("HVRLeftController"))
        {
            HVRLogCore.LOGI(TAG, "current is left controller");
            IControllerHandle ControllerHandle = HvrApi.GetControllerHandle();
            if (ControllerHandle == null) {
    ...
    private void ControllerPositionInit()
    {
        ...
        transform.localPosition = m_HandleInitPosition + HVRLayoutCore.m_CamCtrObj.position;
        m_InitDistance = Vector3.Distance(transform.localPosition, HVRLayoutCore.m_CamCtrObj.position);
    }
    ...
    void Update()
    {
        ...
        if (m_UseNoloController && !m_LeftControllerObj.activeSelf)
        {
            IControllerHandle ControllerHandle = HvrApi.GetControllerHandle();
            if (ControllerHandle != null)
            {
                ...
                if (m_LeftController != null && m_LeftController.IsAvailable())
                {
                    m_LeftControllerObj.SetActive(true);
                }
            }
        }
    ...
    private void UpdateControllerMode()
    {
        ...
        HvrApi.GetControllerHandle().GetValue("prod_model", ref prod_model, 20);
        m_UseNoloController = prod_model.Equals("nolo cv1 air");
        if ((m_IsPhoneControllerMode || m_UseNoloController )&& Application.platform == RuntimePlatform.Android)
        {
            ...
            if (m_UseNoloController && 
                (!m_NoloControllerObj.activeSelf || m_DefaultControllerObj.activeSelf || m_PhoneControllerObj.activeSelf))
            {
                m_IsScreenOn = false;
                m_IsPhoneControllerMode = false;
                HVRLogCore.LOGI(TAG, "switch to nolo controller mode");
                m_PhoneControllerObj.SetActive(false);
                m_NoloControllerObj.SetActive(true);
                m_DefaultControllerObj.SetActive(false);
                m_ControllerObj = m_NoloControllerObj;
            }
            else if (!m_UseNoloController && HVRPluginCore.IsSwitchToController())
            {
                m_IsScreenOn = false;
                m_IsPhoneControllerMode = false;
    ...
    private void UpdateControllerPos()
    {
        ...
        if (m_ControllerType == ControllerType.Controller3DOF)
        {
            ...
            m_Radio = m_UseNoloController ? 0 : m_Radio;
        }
        else
        {
            m_Radio = 0;
